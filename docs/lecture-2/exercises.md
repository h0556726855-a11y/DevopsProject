...
> **Who this is for:** Windows students who cannot use k3d. All exercises and kubectl commands are identical. Only cluster setup, image loading, and browser access differ.

### Start the Cluster

```powershell
winget install minikube   # run once, in PowerShell as Administrator
minikube start
```
...
```powershell
# Build inside Minikube's Docker daemon (recommended):
...
# Or for curl-compatible localhost access (keep running in a separate admin terminal):
minikube tunnel
```
...
All exercises (1, 2, 3, Bonus) run unchanged with minikube once the cluster is up and images are loaded.
